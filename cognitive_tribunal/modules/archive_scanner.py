...
from ..utils.file_utils import (
    Deduplicator, extract_file_metadata
)
...
class ArchiveScanner:
    ...
    # Unsafe paths to block scanning of
    UNSAFE_PATHS_POSIX = {
        '/', '/bin', '/boot', '/dev', '/etc', '/lib', '/lib32', '/lib64',
        '/lost+found', '/proc', '/root', '/run', '/sbin', '/sys', '/usr', '/var'
    }
    ...
    def is_unsafe_path(self, path: Path) -> bool:
        """Check if path is considered unsafe to scan (e.g. system directories)."""
        resolved = path.resolve()
        resolved_str = str(resolved)

        # Block scanning the filesystem root
        if resolved_str == resolved.anchor:
            return True

        # Determine platform-specific unsafe paths
        unsafe_paths = set()
        if os.name == 'posix':
            unsafe_paths = self.UNSAFE_PATHS_POSIX
        elif os.name == 'nt':
            # Add basic Windows system paths
            unsafe_paths = self.UNSAFE_PATHS_NT.copy()
            # Try to get system root and program files from env
            for env_var in ['SystemRoot', 'ProgramFiles', 'ProgramFiles(x86)']:
                val = os.environ.get(env_var)
                if val:
                    unsafe_paths.add(val)
        ...
        for unsafe in unsafe_paths:
            ...
            if resolved_str == unsafe or resolved_str.startswith(f"{unsafe}{os.sep}"):
                return True
    ...
    def scan_directory(self, root_path: str, recursive: bool = True, max_depth: Optional[int] = None) -> Dict:
        ...
        if self.is_unsafe_path(root):
            return {'error': f"Unsafe path detected: {root_path}"}
