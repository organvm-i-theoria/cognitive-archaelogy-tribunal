...
class ArchiveScanner:
    ...
    def is_unsafe_path(self, path: Path) -> bool:
        """
        Check if the path is unsafe to scan (e.g., system root, system directories).
        ...
        try:
            ...
            if os.name == 'posix':
                if path_str in self.UNSAFE_PATHS_POSIX:
                    return True
                # Check if it's a subdirectory of an unsafe path (e.g. /etc/passwd)
                # But we might want to allow scanning subdirectories if the user explicitly requests it?
                # For safety, let's block scanning INSIDE system dirs too, unless we decide otherwise.
                # Actually, usually scanners shouldn't be running in /etc/ at all.
                for unsafe in self.UNSAFE_PATHS_POSIX:
                    if path_str.startswith(unsafe + os.sep) or path_str == unsafe:
                        return True
            ...
            # If we can't resolve or check, fail safe
            return True
    ...
    def scan_directory(self, root_path: str, recursive: bool = True, max_depth: Optional[int] = None) -> Dict:
        ...
        if self.is_unsafe_path(root):
            return {'error': f"Security: Scanning this directory is not allowed: {root}"}
