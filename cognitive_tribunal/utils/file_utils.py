...
class FileHasher:
    ...
    def compute_hash(file_path: Path, algorithm: str = 'sha256') -> str:
        ...
        """
        # Validate algorithm first to match original behavior (raises ValueError)
        if not hasattr(hashlib, 'file_digest'):
            # If file_digest is not available, we need to initialize hasher to check algo validity
            # This preserves the behavior of raising ValueError for bad algo
            hash_func = hashlib.new(algorithm)

        try:
            with open(file_path, 'rb') as f:
                # Optimization: Use file_digest for zero-copy hashing (Python 3.11+)
                if hasattr(hashlib, 'file_digest'):
                    # file_digest raises ValueError if algorithm is unknown, preserving behavior
                    return hashlib.file_digest(f, algorithm).hexdigest()

                # Fallback for older Python versions
                # Read in chunks to handle large files
                # Optimization: Increased buffer size from 8KB to 64KB for better I/O
